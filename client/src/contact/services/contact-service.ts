...
export const createContact = async(body:ICreateContact, token:string)=>{
    return await apiClient.post("/contact", body, {
        headers:{
            "Authorization": `Bearer ${token}`
        }
    })
...
export const updateContact = async(id:number, body:ICreateContact, token:string)=>{
    return await apiClient.put(`/contact/${id}`, body, {
        headers:{
            "Authorization": `Bearer ${token}`
        }
    })
